...
const handleMenu= ()=>{
  ...
  return (
      ...
      <div className={`${menu ? "left-0" : "-left-full"} transition-all duration-500 ease-in-out fixed top-0 bottom-0 ${router.pathname === "/media" ? "bg-coolCharcol" :"bg-white"} min-h-screen w-312px z-40 px-8 py-6`}>
          {router.pathname === "/" ? <div className="w-full h-12" /> : <div className="w-full h-8" />}

          {router.pathname === "/" && (
              <div className="flex items-center justify-between">
                  <button className={`flex items-center space-x-2.5 focus:outline-none  text-sm leading-14px f-f-s`}>
          ...
          <div className="mt-6">
              <p className={`leading-4 text-base ${router.pathname === "/media" ? "text-white" :"text-coolCharcol"}`}>Homepage</p>
              <div className="">
                  {sections.map((item, i) => {
                      return (
                          <div key={i} className={`flex items-center`}>
                              <Link to={item.link}>
                                  <button
                                      ...
                                      className={`flex items-center justify-between leading-4 text-base ${router.pathname === "/media" ? "text-white" :"text-coolCharcol"} mt-6 w-full relative`}
                                  >
                                      <p>{item.name}</p>
                              ...
                              <div className={`${value === i ? "opacity-100 max-h-auto" : "opacity-0 pointer-events-none"}  z-50 absolute  transition-all duration-200 ease-in-out  -right-56 mt-5`}>
                                  <div className={` flex items-center`}>
                                      {" "}
                                      <svg className={`${router.pathname === "/media" ? "text-coolCharcol" :"text-white"} -mr-1 `} width={15} height={18} viewBox="0 0 15 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                          <path d="M14.8428 0.831612L14.6791 17.2853L0.511646 8.91668L14.8428 0.831612Z" fill="currentColor" />
                                      </svg>
                                      <div className={`flex  flex-col space-y-5 w-[200px] ${router.pathname === "/media" ? "bg-coolCharcol" :"bg-white"}  px-4 py-8 mt-20`}>
                                          {item.option.map((_item, j) => {
                                              return <p className={`${router.pathname === "/media" ? "text-white" :"text-coolCharcol"} font-bold`}>{_item}</p>;
                                          })}
                                      </div>
                                  </div>
                              </div>
              ...
              <p className={`${router.pathname === "/media" ? "text-white" :"text-black"} text-base leading-4 f-f-cdb mt-9`}>Join for $0.01/day</p>

              <button className="mt-3 flex items-center justify-between f-f-s text-white text-xs leading-3 bg-sunGolden py-2.5 px-4 rounded shadow-shadowBtn w-full hover:shadow-shadowBtn3 transition-all duration-300 ease-in-out">
                  <p>Join for $0.01/day</p>{" "}
                  <svg width={16} height={16} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <path d="M10 5.33331L12.6667 7.99998" stroke="white" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
              </button>
          </div>
